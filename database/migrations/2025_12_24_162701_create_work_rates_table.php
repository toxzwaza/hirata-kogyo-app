    ...
    public function up()
    {
        Schema::create('work_rates', function (Blueprint $table) {
            ...
            $table->foreignId('work_method_id')
                ...
                ->comment('作業方法ID');
            $table->decimal('rate_employee', 10, 2)->comment('正社員用単価（円/kg）');
            $table->decimal('rate_contractor', 10, 2)->comment('個人事業主用通常単価（円/kg）');
            $table->decimal('rate_overtime', 10, 2)->comment('個人事業主用残業単価（円/kg）');
